...
interface IMigratorChef {
    // Perform LP token migration from legacy PanCybarSwap to CybarSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Cybar LP tokens.
    // CybarSwap must mint EXACTLY the same amount of CybarSwap LP tokens or
    // else something bad will happen. Traditional Cybar does not
...
contract MasterChef is Ownable {
    ...
    function pendingCybar(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cybarReward = multiplier.mul(cybarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCybarPerShare = accCybarPerShare.add(cybarReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        cybar.mint(address(dyce), cybarReward);
        pool.accCybarPerShare = pool.accCybarPerShare.add(cybarReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCybarPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function safeCybarTransfer(address _to, uint256 _amount) internal {
        dyce.safeCybarTransfer(_to, _amount);
    }
