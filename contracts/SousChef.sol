...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many DYCE tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of DYCEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws DYCE tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes Dyce tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = dyce.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 dyceSupply = dyce.balanceOf(address(this));
        if (dyceSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(dyceSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Dyce tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        dyce.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Dyce tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        dyce.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        dyce.safeTransfer(address(msg.sender), user.amount);
